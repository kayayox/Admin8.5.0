...
#include "PatternSequenceCorrelator.hpp"
#include "../utils/StringConversions.hpp"
#include <sstream>

PatternSequenceCorrelator::PatternSequenceCorrelator(const std::string& dbPath)
    : corr(std::make_unique<PatternCorrelator>(dbPath, "_WordType")) {}

WordPattern PatternSequenceCorrelator::makePattern(const std::vector<std::string>& patterns) const {
    WordPattern pat;
...
void PatternSequenceCorrelator::learnWithPrevious(const std::vector<WordType>& pattern) {
...
void PatternSequenceCorrelator::learnNextWordDirect(const std::vector<WordType>& pattern) {
    std::vector<std::string> words;
    for(auto p : pattern){
        words.push_back(wordTypeToString(p));
    }
...
bool PatternSequenceCorrelator::queryNext(const std::string& current,
...
bool PatternSequenceCorrelator::GqueryNext(const std::string& phrase, std::vector<std::pair<WordPattern, double>>& outcomes){
    std::vector<std::string> words;
